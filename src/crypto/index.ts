import { CipherType, EncryptionOptions } from "../ReactNativeLibsignalClient.types";
...
export function encrypt(
  ...
  switch (cipherType) {
    case CipherType.AES256GCM:
      return Aes256Gcm.new(options.key).encrypt(options.text, options.iv, options.aad ?? new Uint8Array());
...
export function decrypt(
  ...
  switch (cipherType) {
    case CipherType.AES256GCM:
      return Aes256Gcm.new(options.key).decrypt(options.text, options.iv, options.aad ?? new Uint8Array());
    case CipherType.AES256CBC:
